...
// Spine Software interface testing
...
void GetReferenceMatrix4x4(vtkMatrix4x4 *Mainmatrix,
                                                         vtkMatrix4x4 *Refmatrix,
                                                         vtkMatrix4x4 *Returnmatrix)
{
  Refmatrix->Invert();
...
vtkMatrix4x4 *GetMatrixTrackingDrfToPatientCt(vtkMatrix4x4 *matrixCtDrfToCtImage,
                                              mitk::Image *inputCtImage,
                                              mitk::NavigationData::Pointer navigationData_ctDrf,
                                              mitk::NavigationData::Pointer navigationData_trackingDrf)
{
  if (navigationData_ctDrf->IsDataValid() && navigationData_trackingDrf->IsDataValid())
  {
    auto toolMatrix = GetVtkMatrix4x4(navigationData_ctDrf);
    auto ReferenceMatrix = GetVtkMatrix4x4(navigationData_trackingDrf);

    auto toolMatrixInRef = vtkMatrix4x4::New();
    GetReferenceMatrix4x4(toolMatrix, ReferenceMatrix, toolMatrixInRef);


    if (inputCtImage == nullptr)
    {
      MITK_INFO << "Input image missing!";
      return nullptr;
    }
    else
    {
      auto tmpmatrix = vtkMatrix4x4::New();
      tmpmatrix = inputCtImage->GetGeometry()->GetVtkMatrix();
      vtkTransform *trans = vtkTransform::New();
      trans->PostMultiply();
      trans->SetMatrix(tmpmatrix);
      trans->Concatenate(matrixCtDrfToCtImage);
      trans->Concatenate(toolMatrixInRef);
  ...
  {
    MITK_INFO << "CtDrf or tracking DRF missing !";
    return nullptr;
